 ...
 ****************************************************/
 ...
 // EDITED BY: Samuel Fisher 2022
 // February 7th, 2022
 // GLM, Trig, and Direction Exercise
 // CSE386
 // Professor Zmuda
...
double normalizeDegrees(double degrees) {
	...
	if (degrees >= 0) {
		...
		double temp = 360 - glm::mod(degrees, 360.0);
	}
	else {
		double temp = glm::mod(degrees, 360.0);
		return temp;
	}
}
...
double distanceFromOrigin(double x, double y) {
	double temp1 = glm::pow(x, 2);
	double temp2 = glm::pow(y, 2);
...
double distanceBetween(double x1, double y1, double x2, double y2) {
	double temp1 = x2 - x1;
	double temp2 = y2 - y1;
	double xRes = glm::pow(temp1, 2);
	double yRes = glm::pow(temp2, 2);
...
double areaOfTriangle(double a, double b, double c) {
	...
	double temp1 = (halfPerim - a);
	double temp2 = (halfPerim - b);
	double temp3 = (halfPerim - c);
	double result = halfPerim * temp1 * temp2 * temp3;
	return sqrt(result);
}
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	...
	double temp2 = (y3 - y1);
	double temp3 = (y1 - y2);
	double temp4 = x1 * temp1;
	double temp5 = x2 * temp2;
	double temp6 = x3 * temp3;
	return (temp4 + temp5 + temp6) / 2;
}
...
double directionInRadians(const dvec2& targetPt) {
	double result = atan2(targetPt.y, targetPt.x);
...
double directionInRadians(double x1, double y1, double x2, double y2) {
	double finalX = x2 - x1;
	double finalY = y2 - y1;
	double result = atan2(finalY, finalX);
	return normalizeRadians(result);
}
...
double cosBetween(const dvec2& v1, const dvec2& v2) {
	...
	double length1 = glm::length(v1);
	double length2 = glm::length(v2);
	double length = length1 * length2;
	double res = dot / length;
	return res;
}
...
double areaOfParallelogram(const dvec3& v1, const dvec3& v2) {
	/* CSE 386 - todo  */
	dvec3 d3 = glm::cross(v1, v2);
	double res = glm::length(d3);
	return res;
}
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	/* CSE 386 - todo  */
	double vec1X = pt2.x - pt1.x;
	double vec1Y = pt2.y - pt1.y;
	double vec1Z = pt2.z - pt1.z;
	dvec3 vec1(vec1X, vec1Y, vec1Z);
	double vec2X = pt3.x - pt2.x;
	double vec2Y = pt3.y - pt2.y;
	double vec2Z = pt3.z - pt2.z;
	dvec3 vec2(vec2X, vec2Y, vec2Z);
	return areaOfParallelogram(vec1, vec2) / 2;
}
...
dvec3 pointingVector(const dvec3& pt1, const dvec3& pt2) {
	/* CSE 386 - todo  */
	dvec3 result(pt2.x - pt1.x, pt2.y - pt1.y, pt2.z - pt1.z);
...
double map(double x, double fromLo, double fromHi, double toLow, double toHigh) {
	/* CSE 386 - todo  */
	double range1 = fromHi - fromLo;
	double range2 = toHigh - toLow;

	double temp = x - fromLo;
	return ((temp)/(range1)) * range2 + toLow;
}
...
vector<double> quadratic(double A, double B, double C) {
	...
	double sqr = B * B;
	double disc = sqr - (4 * (A) * (C));
	if (disc < 0) {
		return result;
	}
	else if (disc == 0) {
		double res = -B + sqrt(disc);
		res /= (2 * (A));
